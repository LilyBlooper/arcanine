...
服务订阅发布 | 配置化发布和引用服务 | 支持通过XML配置的方式发布和导入服务，降低对业务代码的侵入
    see up 		 | 服务自动发现机制     | 支持服务实时自动发现，由注册中心推送服务地址，消费者不需要配置服务提供者地址，服务地址透明化
 see up  | 服务在线注册和去注册 | 支持运行态注册新服务，也支持运行态取消某个服务的注册
服务路由     | 默认提供随机路由、轮询、基于权重的路由策略 | 默认提供常见的路由策略，避免重新造轮子
 see up 		 | 粘滞连接             | 总是向同一个提供方发请求，除非它挂掉，否则不切换
 see up 		 | 路由定制             | 支持用户自定义路由策略，方便扩展
集群容错     | Failover             | 失败自动切换：当出现失败时-重试其他服务器，**通常用于读操作；也可用于幂等写操作**
 see up 		 | Failback			    | 失败自动恢复：后台记录失败请求，定时重发，**通常用于消息通知操作**
 see up 		 | Failfast             | 快速失败，只发起一次调用，失败立即报错，**通常用于非幂等性操作**
服务调用     | 同步调用             | 消费者发起服务调用后，同步阻塞等待服务端响应
 see up 		 | 异步调用             | 消费者发起服务调用后，不阻塞立即返回，由服务端返回应答后异步通知消费者
 see up 			 | 并行调用             | 消费者同时对多个服务提供者批量发起服务调用请求，批量发起请求后，集中等待应答
多协议       | 私有协议             | 支持二进制等私有协议，支持私有协议定制和扩展
 see up            | 公有协议             | 提供 Web Service等公有协议，用于外部服务对接
序列化方式   | 二进制类序列化       | 支持 Thrift，Protocol buffer等二进制协议，提升序列化性能
 see up             | 文本类序列化         | 支持 JSON，XML等文本类型的序列化方式，提升通用性和可读性
统一配置     | 本地静态配置         | 安装部署修改一次，运行态不修改的配置，可以存放到本地配置文件中
 see up            | 基于配置中心的动态配置 | 运行态需要调整的参数，统一放到配置中心（服务注册中心），修改之后统一下发，实时生效
